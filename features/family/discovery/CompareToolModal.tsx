...
import { X, MapPin, Phone, CheckCircle, ArrowRight } from 'lucide-react';
    ...
    const fetchComparables = async () => {
      ...
      try {
        ...
        const scoredCity = cityResults.map((facility: any) => {
          ...
          const score = (hasCareOverlap ? 2 : 0) + (hasPriceOverlap ? 1 : 0);
          return { ...facility, _score: score };
        });
  ...
  return (
    <div className="fixed inset-0 z-[70]">
      ...
      <div className="relative max-w-6xl mx-auto mt-12 mb-10 bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-[#f6f1ea]">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Comparison Tool</p>
            <h2 className="text-xl font-semibold text-slate-900">Compare Similar Homes</h2>
          </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6 px-6 py-6">
          <section className="lg:col-span-5 space-y-4">
            <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
              <p className="text-xs uppercase tracking-[0.2em] text-slate-400 mb-2">Current Facility</p>
              <div className="flex gap-4">
                <div className="w-24 h-24 rounded-xl overflow-hidden bg-slate-100 flex items-center justify-center">
                  {baseFacility.image ? (
                    ...
                    <span className="text-2xl font-bold text-slate-400">{baseFacility.name?.charAt(0)}</span>
                  )}
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-slate-900">{baseFacility.name}</h3>
                  <p className="text-sm text-slate-600 flex items-center gap-1">
                    <MapPin size={14} /> {baseFacility.city}, {baseFacility.state}
                  </p>
                  {baseFacility.phone && (
                    <p className="text-sm text-slate-600 flex items-center gap-1 mt-1">
                      <Phone size={14} /> {baseFacility.phone}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
              <h4 className="text-sm font-semibold text-slate-900 mb-3">How we pick comparables</h4>
              <ul className="text-sm text-slate-600 space-y-2">
                <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-slate-700 mt-0.5" />Same city first</li>
                <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-slate-700 mt-0.5" />Matching care types</li>
                <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-slate-700 mt-0.5" />Similar price range</li>
                <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-slate-700 mt-0.5" />Fallback to same state</li>
              </ul>
            </div>
          </section>
              ...
              {selected && (
                <div className="mt-6 border-t border-slate-200 pt-4">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4 text-sm">
                    ...
                    <div className="bg-slate-50 rounded-xl p-4">
                      <p className="text-xs uppercase text-slate-400 mb-2">{selected.name}</p>
                      ...
                      <p className="text-slate-700"><strong>Care:</strong> {getCareTypes(selected).join(', ') || 'Assisted Living'}</p>
